...
def main():
    ...
    if args.tasks is not None:
        ...
        if args.tasks == "wikitext":
            ...
            # 🚀 FIXED: Proper perplexity calculation like llama.cpp
            total_nll = 0.0
            total_count = 0
            
            print(f"📊 Evaluating perplexity over {nsamples} chunks (seqlen={model.seqlen})")
            ...
            for i in tqdm.tqdm(range(nsamples), desc="evaluating..."):
                ...
                # 🔧 FIXED: Process each token individually like llama.cpp
                # Use only the latter half for evaluation (like llama.cpp's n_ctx/2)
                first_eval_pos = model.seqlen // 2 if i == 0 else 1  # Skip first token only for first chunk
                
                for pos in range(first_eval_pos, model.seqlen - 1):  # -1 because we predict next token
                    # Get logits for current position
                    logits = lm_logits[0, pos, :].float()  # [vocab_size]
                    target_token = batch[0, pos + 1].item()  # Next token is the target
                    
                    # 🚀 FIXED: Compute log_softmax manually (like llama.cpp)
                    # Numerical stability: subtract max logit
                    max_logit = torch.max(logits).item()
                    logits_shifted = logits - max_logit
                    sum_exp = torch.sum(torch.exp(logits_shifted)).item()
                    log_sum_exp = max_logit + torch.log(torch.tensor(sum_exp)).item()
                    
                    # Compute negative log-likelihood for target token
                    target_log_prob = logits[target_token].item() - log_sum_exp
                    nll = -target_log_prob
                    
                    total_nll += nll
                    total_count += 1
                
                # 🔧 Progress reporting (like llama.cpp)
                if (i + 1) % 10 == 0 or i == 0:
                    current_ppl = torch.exp(torch.tensor(total_nll / total_count)).item()
                    print(f"[{i+1}/{nsamples}] Current PPL: {current_ppl:.4f}")

            # 🚀 FIXED: Correct perplexity calculation
            average_nll = total_nll / total_count
            ppl = torch.exp(torch.tensor(average_nll)).item()
            
            print(f"\n✅ CORRECTED PERPLEXITY CALCULATION:")
            print(f"📊 Total tokens evaluated: {total_count}")
            print(f"📊 Average NLL: {average_nll:.6f}")
            print(f"🎯 Final Perplexity: {ppl:.4f}")
            
            # 🔍 COMPARISON: Show what the old (incorrect) calculation would give
            print(f"\n🔍 COMPARISON WITH OLD METHOD:")
            old_method_nll = total_nll  # This simulates what the old method was doing wrong
            old_method_ppl = torch.exp(torch.tensor(old_method_nll / (nsamples * model.seqlen))).item()
            print(f"❌ Old (incorrect) method would give: {old_method_ppl:.4f}")
            print(f"✅ New (correct) method gives: {ppl:.4f}")
            print(f"📈 Difference: {abs(ppl - old_method_ppl):.4f}")
